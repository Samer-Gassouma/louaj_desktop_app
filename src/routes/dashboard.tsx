...
function SupervisorDashboard() {
  ...
  return (
    <div className="flex flex-col h-screen w-full overflow-y-auto p-6 space-y-6">
      {/* Header and Income Cards */}
      <div className="space-y-4">
        <div>
          <h1 className="text-3xl font-bold">Supervisor Dashboard</h1>
          <p className="text-muted-foreground">
            Station income overview and transaction management
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-4 gap-4">
          <Card className="p-4 lg:p-6 bg-gradient-to-br from-green-50 to-green-100 border-green-200">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-green-800">Today's Income</p>
              ...
              <div className="w-10 h-10 lg:w-12 lg:h-12 bg-green-700 rounded-lg flex items-center justify-center">
          ...
          <Card className="p-4 lg:p-6 bg-gradient-to-br from-blue-50 to-blue-100 border-blue-200">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-blue-800">This Month's Income</p>
              ...
              <div className="w-10 h-10 lg:w-12 lg:h-12 bg-blue-800 rounded-lg flex items-center justify-center">
          ...
          <Card className="p-4 lg:p-6 bg-gradient-to-br from-purple-50 to-purple-100 border-purple-200">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-purple-800 mt-1">All time</p>
              </div>
              <div className="w-10 h-10 lg:w-12 lg:h-12 bg-purple-800 rounded-lg flex items-center justify-center">
          ...
          <Card className="p-4 lg:p-6 bg-gradient-to-br from-orange-50 to-orange-100 border-orange-200">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-orange-800">Average Transaction</p>
                <p className="text-2xl lg:text-3xl font-bold text-orange-800">
                  {formatCurrency(
                    transactions.length > 0
                      ? (todayIncome + monthIncome) / transactions.length
                      : 0
                  )}
                </p>
                <p className="text-xs text-orange-800 mt-1">Per transaction</p>
              </div>
              <div className="w-10 h-10 lg:w-12 lg:h-12 bg-orange-800 rounded-lg flex items-center justify-center">
                <TrendingUp className="h-5 w-5 lg:h-6 lg:w-6 text-white" />
              </div>
            </div>
          </Card>
...
function RegularDashboard() {
  ...
  const availableSeats = queueStats.reduce(
    (sum, stat) => sum + stat.availableSeats,
    0
  );
  const averageOccupancy =
    queueStats.length > 0
      ? queueStats.reduce((sum, stat) => sum + stat.occupancyRate, 0) /
      queueStats.length
      : 0;
                ...
                >
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      {stat.vehicleCount} vehicles
                    </p>
                  </div>
                  <div className="text-right">
                    <p className="font-semibold">
                      {stat.availableSeats}/{stat.totalSeats}
                    </p>
                    <p className="text-sm text-muted-foreground">
                      {stat.occupancyRate.toFixed(0)}% full
                    </p>
                  </div>
                </div>
              ))}
